...
public class CircleSeekBar extends View {
    ...
    private static final String INSTANCE_UNREACHED_OVERSCROLL_COLOR = "unreached_overscroll_color";
    private static final String INSTANCE_UNREACHED_OVERSCROLL_WIDTH = "unreached_overscroll_width";
    ...
    private static final String INSTANCE_WHEEL_SCROLL_INFINITE = "wheel_scroll_infinite";
    ...
    private Paint mWheelOverscrollPaint;
    ...
    private int mReachedColor, mUnreachedColor, mUnreachedOverscrollColor;
    private float mReachedWidth, mUnreachedWidth, mUnreachedOverscrollWidth;
    ...
    private boolean isScrollInfinite;
    ...
    private void initPaints() {
        mDefShadowOffset = getDimen(R.dimen.circleseekbar_shadow_offset);
        /**
         * 圆环画笔
         */
        ...
        mWheelOverscrollPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mWheelOverscrollPaint.setColor(mUnreachedOverscrollColor);
        mWheelOverscrollPaint.setStyle(Paint.Style.STROKE);
        mWheelOverscrollPaint.setStrokeWidth(mUnreachedOverscrollWidth);
        if (isHasWheelShadow) {
            mWheelOverscrollPaint.setShadowLayer(mWheelShadowRadius, mDefShadowOffset,
    ...
    private void initAttrs(AttributeSet attrs, int defStyle) {
        ...
        mReachedColor = a.getColor(R.styleable.CircleSeekBar_wheel_reached_color, getColor(R.color.circleseekbar_reached));
        mUnreachedColor = a.getColor(R.styleable.CircleSeekBar_wheel_unreached_color,
                getColor(R.color.circleseekbar_unreached));
        mUnreachedWidth = a.getDimension(R.styleable.CircleSeekBar_wheel_unreached_width,
                getDimen(R.dimen.circleseekbar_width));
        mUnreachedOverscrollColor = a.getColor(R.styleable.CircleSeekBar_wheel_unreached_overscroll_color,
                getColor(R.color.circleseekbar_overscroll));
        mUnreachedOverscrollWidth = a.getDimension(R.styleable.CircleSeekBar_wheel_unreached_overscroll_width,
                mUnreachedWidth);
        ...
        mPointerColor = a.getColor(R.styleable.CircleSeekBar_wheel_pointer_color, getColor(R.color.circleseekbar_pointer));
        ...
        if (isHasWheelShadow) {
            mWheelShadowRadius = a.getDimension(R.styleable.CircleSeekBar_wheel_shadow_radius,
                    getDimen(R.dimen.circleseekbar_shadow_radius));
        }
        ...
        isScrollInfinite = a.getBoolean(R.styleable.CircleSeekBar_wheel_scroll_infinite, true);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isHasCache && rounds == 0) {
            if (mCacheCanvas == null) {
                buildCache(centerX, centerY, wheelRadius);
            }
            canvas.drawBitmap(mCacheBitmap, 0, 0, null);
        } else {
            if (!isScrollInfinite || rounds == 0)
                canvas.drawCircle(centerX, centerY, wheelRadius, mWheelPaint);
            else if (isScrollInfinite && rounds > 0)
                canvas.drawCircle(centerX, centerY, wheelRadius, mWheelOverscrollPaint);
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (isCanTouch && (event.getAction() == MotionEvent.ACTION_MOVE || isTouch(x, y))) {
            ...
            } else if (isScrollInfinite) {
                if (mCurAngle > 270 && angle < 90) {
                    // Wheel overscrolling 0 from left to right
                    // Increment round counter
                    rounds++;
                    mCurAngle = angle;
                } else if (mCurAngle < 90 && angle > 270) {
                    // Wheel overscrolling 0 from right to left
                    if (rounds == 0) {
                        // Stop at 0
                        mCurAngle = 0;
                        cos = -1;
                    } else {
                        // Allow wheel past zero and
                        // decrement round counter
                        rounds = rounds > 0 ? rounds - 1 : 0;
                        mCurAngle = angle;
                    }
                } else {
                    // A normal scroll
    ...
    private void refershPosition() {
        mCurAngle = (double) (mCurProcess - (rounds * mMaxProcess)) / mMaxProcess * 360.0;
    ...
    protected Parcelable onSaveInstanceState() {
        ...
        bundle.putInt(INSTANCE_UNREACHED_OVERSCROLL_COLOR, mUnreachedOverscrollColor);
        bundle.putFloat(INSTANCE_UNREACHED_OVERSCROLL_WIDTH, mUnreachedOverscrollWidth);
        ...
        bundle.putBoolean(INSTANCE_WHEEL_SCROLL_INFINITE, isScrollInfinite);
        return bundle;
    }
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        if (state instanceof Bundle) {
            ...
            mUnreachedOverscrollColor = bundle.getInt(INSTANCE_UNREACHED_OVERSCROLL_COLOR);
            mUnreachedOverscrollWidth = bundle.getFloat(INSTANCE_UNREACHED_OVERSCROLL_WIDTH);
            ...
            isScrollInfinite = bundle.getBoolean(INSTANCE_WHEEL_SCROLL_INFINITE);
            initPaints();
        } else {
    ...
    private int getSelectedValue() {
        return Math.round(mMaxProcess * ((float) mCurAngle / 360) + mMaxProcess * rounds);
    }
    ...
    public void setCurProcess(int curProcess) {
        ...
        this.mCurProcess = isScrollInfinite ? curProcess : Math.max(curProcess, mMaxProcess);
        this.rounds = (int) curProcess / mMaxProcess;
